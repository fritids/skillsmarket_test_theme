...
function bones_filter_title( $title, $sep ) {
  ...
  if ( $paged >= 2 || $page >= 2 ) {
    $title = "$title $sep " . sprintf( __( 'Page %s', 'skillsmarket' ), max( $paged, $page ) );
  }
    ...
    <article id="comment-<?php comment_ID(); ?>" class="clearfix">
      <div class="comment-author vcard row-fluid clearfix">
        ...
        <div class="span9 comment-text">
          ...
          <?php edit_comment_link(__('Edit','skillsmarket'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>') ?>

          <?php if ($comment->comment_approved == '0') : ?>
            <div class="alert-message success">
              <p><?php _e('Your comment is awaiting moderation.','skillsmarket') ?></p>
            </div>
          <?php endif; ?>
...
function list_pings($comment, $args, $depth) {
  $GLOBALS['comment'] = $comment; ?>
  <li id="comment-<?php comment_ID(); ?>"><i class="icon icon-share-alt"></i>&nbsp;<?php comment_author_link(); ?><?php 
}
...
function bones_wpsearch( $form ) {
  ...
  <label class="screen-reader-text" for="s">' . __('Search for:', 'skillsmarket') . '</label>
  <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Search the Site..." />
  <input type="submit" id="searchsubmit" value="'. esc_attr__('Search','skillsmarket') .'" />
  </form>';
  return $form;
} // don't remove this bracket!
...
function custom_password_form() {
	...
	' . '<p>' . __( "This post is password protected. To view it please enter your password below:" ,'skillsmarket') . '</p>' . '
	<label for="' . $label . '">' . __( "Password:" ,'skillsmarket') . ' </label><div class="input-append"><input name="post_password" id="' . $label . '" type="password" size="20" /><input type="submit" name="Submit" class="btn btn-primary" value="' . esc_attr__( "Submit",'skillsmarket' ) . '" /></div>
	</form></div>
	';
	return $o;
}
...
function show_homepage_meta_box() {  
  ...
  foreach ( $custom_meta_fields as $field ) {
    ...
    switch($field['type']) {
      // text
      case 'text':
        echo '<input type="text" name="'.$field['id'].'" id="'.$field['id'].'" value="'.$meta.'" size="60" /><br /><span class="description">'.$field['desc'].'</span>';
      break;
      // textarea
      case 'textarea':
        echo '<textarea name="'.$field['id'].'" id="'.$field['id'].'" cols="80" rows="4">'.$meta.'</textarea><br /><span class="description">'.$field['desc'].'</span>';
      break;
    } //end switch
    echo '</td></tr>';
  } // end foreach
